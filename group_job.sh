...
curlToFile $TMP_BGM_RAUKEN_TOPICLIST_URL $G_GIT_REPO_DIR/group/rakuen_topic_list.html

topic_list=`grep -Po '(?<=href="/rakuen/topic/group/)[0-9]+' $G_GIT_REPO_DIR/group/rakuen_topic_list.html | sort | uniq`
#Clear
...
topic_list=`grep -Po '(?<=href="/rakuen/topic/group/)[0-9]+' $G_GIT_REPO_DIR/group/rakuen_topic_list.html | sort | uniq | shuf`
print_info TOPIC LIST: $topic_list

for i in $topic_list
do
	print_info archiving group topic $i
...
currentTimeMills
git_commit_msg+="| $G_RET"
exec_cmd_nobail_naked "rm -rf group/rakuen_topic_list.html"
exec_cmd_nobail_naked "$G_GIT_CMD add group/*"
