...
G_GIT_REPO_DIR=${BGM_GIT_REPO:-~/source/bgm-archive-blog-2}
...
#for (( i=$(expr $last_blog_id + 1);i<=213200;i++ ))
for (( i=190186;i<=190475;i++ ))
do
	echo archiving $i
	...
	mkdir -p $output_dir
	curl -sL -b $BGM_COOKIE_FILE -A "$BGM_UA" --connect-timeout 10 --max-time 20 --retry 6 --retry-delay 3 --retry-max-time 60 --output $output_loc "https://bgm.tv/blog/$i"
