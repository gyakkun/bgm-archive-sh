...
G_GIT_REPO_DIR=${BGM_GIT_REPO:-~/source/bgm-archive-blog-1}
...
export BGM_UA='Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36 Edg/112.0.1722.23'
...
# for (( i=$(expr $last_blog_id + 1);i<=106600;i++ ))
for (( i=56515;i<=56820;i++ ))
do
	echo archiving $i
	...
	mkdir -p $output_dir
	curl -sL -b $BGM_COOKIE_FILE -A "$BGM_UA" --connect-timeout 10 --max-time 20 --retry 6 --retry-delay 3 --retry-max-time 60 --output $output_loc "https://bangumi.tv/blog/$i"
